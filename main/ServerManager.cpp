...
void ServerManager::begin() {
  ...
  if (initWiFi()) {
    ...
    String WiFiAP = "ESP32_" + generateDeviceID(); // 拼接SSID
    WiFi.softAP(WiFiAP.c_str(), NULL);
...
// Generate device ID
String ServerManager::generateDeviceID() {
  uint64_t chipid = ESP.getEfuseMac(); // The chip ID is essentially its MAC address
  char deviceID[5];
  sprintf(deviceID, "%04X", (uint16_t)(chipid >> 32));
...
String ServerManager::getAPI() {
  return (WiFi.softAPIP().toString()); // 將 IPAddress 轉換為 String
}
